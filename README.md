...
General and Accounts settings, saved ChatGPT/API accounts with automatic model
detection, direct account activation with Safe Switch restore points and
rollback, third-party Provider mode for ChatGPT logins backed by saved API
accounts, opening the bundled Session Manager, automatic and manual Repair Now
for local official thread metadata, rollout provider synchronization for
historical session JSONL files during Safe Switch and Provider-mode entry, Codex
